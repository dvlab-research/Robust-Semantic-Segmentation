...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = DeepLabV3_DDCAT(layers=args.layers, classes=args.classes, zoom_factor=args.zoom_factor, criterion=criterion, BatchNorm=BatchNorm)
    optimizer = torch.optim.SGD(
        [{'params': model.layer0.parameters()},
         ...
         {'params': model.aspp.parameters(), 'lr': args.base_lr * 10},
         {'params': model.cls1.parameters(), 'lr': args.base_lr * 10},
         {'params': model.cls2.parameters(), 'lr': args.base_lr * 10},
         {'params': model.mask1.parameters(), 'lr': args.base_lr * 10},
         {'params': model.aux_cls1.parameters(), 'lr': args.base_lr * 10}],
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_train, mIoU_train, mAcc_train, allAcc_train = train(train_loader, model, optimizer, epoch, criterion)
        if main_process():
        ...
        if args.evaluate:
            loss_val, mIoU_val, mAcc_val, allAcc_val = validate(val_loader, model, criterion,  optimizer)
            if main_process():
...
def FGSM(input, target, model, criterion, optimizer, clip_min, clip_max, eps=0.06):
    ...
    indicate_temp = torch.zeros_like(target)
    _, loss1, loss2, _, result_normal = model(input_variable, y_target=target, indicate_map=indicate_temp, indicate=1)
    loss_our = criterion(result_normal, target)
    ...
    adver_result_max, _, _, _, _ = model(adversarial_example.detach(), indicate=1)
    ...
    ################################################################################
    return adversarial_example, adver_result_max
...
def BIM(input, target, model, criterion, optimizer, eps=0.03, k_number=3):
    ...
    for mm in range(k_number):
        adversarial_example, result_max = FGSM(adversarial_example, target, model, criterion,
                                               optimizer, clip_min, clip_max, eps=eps*1.0/k_number)
        if mm == 0:
            result_max_origin = result_max.clone()
        adversarial_example = adversarial_example.detach()
        adversarial_example.requires_grad = True
        model.zero_grad()
    ####################################################################################################
    adver_result_max, _, _, _, adver_result_normal = model(adversarial_example.detach(),
                                                           indicate_map=indicate_temp, indicate=1)
    adver_result_normal_max = adver_result_normal.max(1)[1]
    ####################################################################################################
    normal_max = result_max_origin.detach().cpu().numpy()
    adver_max = adver_result_max.detach().cpu().numpy()
    target_numpy = target.clone().detach().cpu().numpy()
    ####################################################################################################
    y_adver_indicate = np.zeros_like(target_numpy)
    y_adver_indicate[adver_max != normal_max] = 1
    y_adver_indicate = torch.Tensor(y_adver_indicate).long().cuda()
    ####################################################################################################
    model.zero_grad()
    adversarial_example = adversarial_example.detach()
    model.train()

    return adversarial_example, y_adver_indicate, adver_result_normal_max
...
def train(train_loader, model, optimizer, epoch, criterion):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        target_adver = target.clone().detach()
        adver_example, indicate_FGSM, output = BIM(input, target, model, criterion, optimizer)
        indicate_temp = torch.zeros_like(target).cuda()
        ...
        # ours
        final_indicate = torch.cat([indicate_FGSM[0:batch_size // 2], indicate_temp[batch_size // 2:batch_size]], dim=0)

        _, main_loss, aux_loss, _, result_normal = model(final_input, y_target=target, indicate_map=final_indicate)
